...
| `amr_navigation/explore` (`amr_navigation_explore`) | Autonomous map building. Finds frontiers (free cells touching unmapped space) on the live `/map`, ranks them, and drives to them through Nav2 until the reachable area is mapped. Exposes one action, `explore_area`; `frontier_search.py` is the grid maths, ROS-free and unit-tested. |
...
- `amr_navigation/move`, `amr_navigation/map`, `amr_navigation/imu`,
  `amr_navigation/lidar` and `amr_navigation/explore` are separate colcon
  packages (`amr_navigation_move`, `amr_navigation_map`, `amr_navigation_imu`,
  `amr_navigation_lidar`, `amr_navigation_explore`) nested under one directory
  — colcon discovers `package.xml` recursively, so this is valid and keeps the
  Navigation grouping visible in the tree.
